        ...
        try {
            ...
            if($verifiedToken){
                $leave = new LeavesListModel;
                    $leave_info = $leave->get_leaves_list($designation,$emp_id);
                    $resp = array('leave_info' => $leave_info);
                    $this->response($resp, 200);
                    if(empty($items)){
                        $leave = new LeavesListModel;
                        $total_rows = $leave->count_rows($table,$designation,$emp_id);
                        $leave_rows = $leave->get_rows($limit, $start, $table,$designation,$emp_id);
                    ...
                    elseif(empty($items[0]->value)){
                        ...
                        $leave = new LeavesListModel;
                        $total_rows = $leave->count_filtered_rows($search, $col, $table, $operator,$designation,$emp_id);
                        $leave_rows = $leave->get_filtered_rows($limit, $start, $search, $col, $operator, $table,$designation,$emp_id);
