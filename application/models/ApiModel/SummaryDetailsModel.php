...
class SummaryDetailsModel extends CI_Model
{
    ...
    public function get_summary_detail($schedule_id)
    {
        $query = $this->db->get_where('vw_attendance_summary', array('id' => $schedule_id));
        if($query->num_rows()>0){
            return $query->result_array();
        }
    }
